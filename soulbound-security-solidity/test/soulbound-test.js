...
describe("Soulbound Token Test", function () {
    ...
    beforeEach(async function () {
        ...
        const Soulbound = await ethers.getContractFactory("SoulboundSecurity");
        soulbound = await Soulbound.deploy();
    });

    it("should create a project and mint a soulbound token", async () => {
        //Mint token ID 0 to owner address
        await soulbound.createProject("testProject", owner.address);
        await soulbound.mint(owner.address);
    ...
    it("should revert when trying to transfer via transferFrom", async () => {
        //Mint token ID 0 to owner address
        await soulbound.mint("testProject", owner.address);
